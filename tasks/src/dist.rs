...
pub struct Distribution {
    zip_file: PathBuf,
    artifacts: Vec<(PathBuf, String, u32)>,
    files: Vec<(PathBuf, String, u32)>,
}
...
impl Distribution {
    ...
    pub fn with_artifact<P: AsRef<Path>, S: Into<String>>(
        mut self,
        path: P,
        entry: S,
        mode: u32,
    ) -> Self {
        self.artifacts.push((target_path("release").join(path), entry.into(), mode));
        self
    }

    /// Add a regular file (e.g. a configuration file or a README).
    pub fn with_file<P: AsRef<Path>, S: Into<String>>(
        mut self,
        path: P,
        entry: S,
        mode: u32,
    ) -> Self {
        self.files.push((project_root().join(path), entry.into(), mode));
        self
    }

    /// Run a plain `cargo build` command.
    pub fn cargo_build(self, args: &[&str]) -> Result<Self> {
        let mut command = Command::new("cargo");
        command.arg("build").args(args).env("CARGO_XTASK_DIST", "true");
    ...
    pub fn build(self, args: &[&str]) -> Result<Self> {
        ...
        for (src, dst, mode) in self.artifacts.iter().chain(self.files.iter()) {
